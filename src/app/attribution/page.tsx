...
const engineBadge: Record<string, { bg: string; color: string; border: string }> = {
  ChatGPT: { bg: "rgba(16,185,129,0.1)", color: "#34d399", border: "rgba(16,185,129,0.2)" },
  Perplexity: { bg: "rgba(99,102,241,0.1)", color: "#818cf8", border: "rgba(99,102,241,0.2)" },
  Gemini: { bg: "rgba(245,158,11,0.1)", color: "#fbbf24", border: "rgba(245,158,11,0.2)" },
  Claude: { bg: "rgba(236,72,153,0.1)", color: "#f472b6", border: "rgba(236,72,153,0.2)" },
  "Google AI": { bg: "rgba(59,130,246,0.1)", color: "#60a5fa", border: "rgba(59,130,246,0.2)" },
};
...
const trendIcon = (trend: string) => {
  switch (trend) {
    ...
    default: return <Minus className="w-3.5 h-3.5" style={{ color: "#484f68" }} />;
  }
};
...
const CustomTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div className="rounded-lg px-3 py-2.5 text-xs" style={{ background: "#0d0e1a", border: "1px solid rgba(99,102,241,0.2)" }}>
      <div className="font-medium mb-1" style={{ color: "#9ca3af" }}>{label}</div>
      <div className="flex items-center gap-2">
        <span style={{ color: "#6b7280" }}>Citations:</span>
        <span className="mono font-bold" style={{ color: "#a5b4fc" }}>{payload[0].value}</span>
      </div>
    </div>
  );
};
...
export default function AttributionPage() {
  ...
  const chartData = [...sourceAttributions]
    ...
    .slice(0, 8)
    .map((s) => ({
      name: s.page.length > 22 ? s.page.slice(0, 22) + "…" : s.page,
      ...
      <div className="flex items-center justify-between mb-8 animate-fade-in-up">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-xl font-semibold" style={{ color: "#e2e8f0" }}>Source Attribution</h1>
          </div>
      ...
      <div className="grid grid-cols-4 gap-3 mb-6 animate-fade-in-up-1">
        <div className="glass-card rounded-xl p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-[10px] tracking-widest font-semibold" style={{ color: "#343650" }}>TOTAL CITATIONS</span>
          </div>
          <div className="mono text-3xl font-bold text-white">{totalCitations}</div>
        </div>
        <div className="glass-card rounded-xl p-5">
          <div className="flex items-center gap-2 mb-3">
            <TrendingUp className="w-3.5 h-3.5" style={{ color: "#10b981" }} />
            <span className="text-[10px] tracking-widest font-semibold" style={{ color: "#343650" }}>TRENDING UP</span>
          </div>
          <div className="mono text-3xl font-bold" style={{ color: "#10b981" }}>{upTrending}</div>
        </div>
        <div className="glass-card rounded-xl p-5">
          <div className="flex items-center gap-2 mb-3">
            <span className="text-[10px] tracking-widest font-semibold" style={{ color: "#343650" }}>PAGES TRACKED</span>
          </div>
          <div className="mono text-3xl font-bold text-white">{sourceAttributions.length}</div>
        </div>
        <div className="glass-card rounded-xl p-5">
          <div className="text-[10px] tracking-widest font-semibold mb-2" style={{ color: "#343650" }}>TOP CITED PAGE</div>
          <div className="text-sm font-medium truncate" style={{ color: "#818cf8" }}>{topPage.page}</div>
          <div className="mono text-xs mt-1" style={{ color: "#484f68" }}>{topPage.citationCount} citations</div>
        </div>
      </div>

      {/* Chart */}
      <div className="glass-card rounded-xl p-6 mb-5 animate-fade-in-up-2">
        <div className="text-xs font-semibold tracking-widest mb-5" style={{ color: "#484f68" }}>
        ...
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={chartData} barSize={22}>
            ...
            <XAxis
              ...
              height={50}
            />
            <YAxis tick={{ fontSize: 10, fill: "#484f68" }} axisLine={false} tickLine={false} />
            ...
            <tr style={{ borderBottom: "1px solid rgba(99,102,241,0.06)" }}>
              {[
                ...
                <th
                  ...
                  style={{ color: "#343650", cursor: col.key ? "pointer" : "default" }}
                  ...
                  <span className="flex items-center gap-1">
                    ...
                    {col.key && (
                      <ArrowUpDown
                        ...
                        style={{ color: sortKey === col.key ? "#6366f1" : "#343650" }}
                      />
                    )}
                  </span>
                </th>
            ...
            {sorted.map((s, i) => (
              ...
              >
                <td className="px-5 py-3.5">
                  <div className="font-medium text-sm" style={{ color: "#c9d1e0" }}>{s.page}</div>
                  <div
                    ...
                    {s.citedBy.map((engine) => {
                      ...
                      return (
                        <span
                          ...
                          style={eb ? { background: eb.bg, color: eb.color, border: `1px solid ${eb.border}` } : { background: "rgba(107,114,128,0.1)", color: "#9ca3af" }}
                        >
                          {engine}
                        </span>
                      );
                ...
                <td className="px-5 py-3.5">
                  <div className="flex items-center gap-2">
                    <div
                      ...
                      style={{
                        width: `${Math.round((s.citationCount / 53) * 48)}px`,
                        background: "linear-gradient(90deg, #6366f1, #8b5cf6)",
                        minWidth: "4px",
                      }}
                    />
                    <span className="mono text-sm font-bold text-white">{s.citationCount}</span>
                  </div>
                </td>
                <td className="px-5 py-3.5">
                  {trendIcon(s.trend)}
                ...
                <td className="px-5 py-3.5">
                  <span className="mono text-xs" style={{ color: "#484f68" }}>{s.lastCited}</span>
                </td>
              </tr>
            ))}
          </tbody>
